...
equation
  ...
  connect(fermiDiracEoS.c_v, add.u2) annotation(
    Line(points = {{-38, -26}, {-30, -26}, {-30, -6}, {-22, -6}}, color = {0, 0, 127}));
  annotation(
    ...
    Documentation(info = "<html><head></head><body><blockquote><p>Similar to the HeatCapacitor block in the MSL, except that the specific heat capacity is allowed to vary according to the temperature, c_v(T)=c_p(T). The heat capacity is the sum of those determined via the Debye model (lattice phonons) and the Fermi-Dirac distribution (conduction electrons). This approach is fitting for many solids, both conducting (N<sub>e</sub>&gt;0) and insulating (N<sub>e</sub>=0).</p></blockquote>
</body></html>"),
  Diagram(coordinateSystem(extent = {{-100, 40}, {0, -40}})));
